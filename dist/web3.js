...
const swapTokensShadow_1 = __importDefault(require("./swapTokensShadow"));
...
async function manageLiquidityPosition(poolAddress) {
    ...
    try {
        ...
        if (!shouldMintNewPosition) {
            ...
            console.log(`- Fees Owed: ${ethers_1.ethers.utils.formatUnits(tokensOwed0, 6)} USDC.e, ${ethers_1.ethers.utils.formatUnits(tokensOwed1, 18)} ${poolName}`);
        ...
        if (shouldMintNewPosition) {
            ...
            if (token1Balance > optimalAmounts.token1Amount) {
                ...
                // return false;
                await (0, swapTokensShadow_1.default)(token1.address, token0.address, token1Delta, token1Decimals);
            }
                ...
                // return false;
                await (0, swapTokensShadow_1.default)(token0.address, token1.address, token0Delta, token0Decimals);
            }
            // Refresh balances
...
async function runWeb3Tasks() {
    try {
        const x33Handled = await manageLiquidityPosition(constants_1.ADDRESSES.USDC_X33_POOL);
