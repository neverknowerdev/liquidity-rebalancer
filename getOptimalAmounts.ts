...
interface OptimalAmounts {
    token0Amount: bigint;
    token0AmountNumber: number;
    token1Amount: bigint;
    token1AmountNumber: number;
}

function getOptimalAmounts(
    pool: Pool,
    price: number,
    availableAmount0: number,
    availableAmount1: number,
    tickLower: number,
    tickUpper: number,
    slippage?: number
): OptimalAmounts {
    ...
    const token0amount = Number(ethers.utils.formatUnits(availableAmount0.toString(), token0decimals));
    const token1amount = Number(ethers.utils.formatUnits(availableAmount1.toString(), token1decimals));
...
function amountToBigInt(amount: number, decimals: number): bigint {
    return ethers.utils.parseUnits((amount).toFixed(decimals), decimals).toBigInt()
}
//
...
export default getOptimalAmounts;