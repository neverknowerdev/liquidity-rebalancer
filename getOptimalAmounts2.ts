...
export function getOptimalAmounts2(amount0: bigint, amount1: bigint, price: number, lowerTick: bigint, upperTick: bigint, currentTick: bigint): OptimalAmounts {
...
const Q96: bigint = BigInt(2) ** BigInt(96);
...
function liquidity0(amount: bigint, pa: bigint, pb: bigint): bigint {
...
function liquidity1(amount: bigint, pa: bigint, pb: bigint): bigint {
    let [lower, upper] = pa > pb ? [pb, pa] : [pa, pb];
    // amount * Q96 / (pb - pa)
...
function calcAmount0(liq: bigint, pa: bigint, pb: bigint): bigint {
    let [lower, upper] = pa > pb ? [pb, pa] : [pa, pb];
    // liq * Q96 * (pb - pa) / pa / pb
...
function calcAmount1(liq: bigint, pa: bigint, pb: bigint): bigint {
    let [lower, upper] = pa > pb ? [pb, pa] : [pa, pb];
    // liq * (pb - pa) / Q96
