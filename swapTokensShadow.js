...
const {ADDRESSES, toTokenName}= require('./constants');
...
async function swapTokensShadow(tokenIn, tokenOut, amountIn, decimalsIn) {
    ...
    console.log(`swapping ${Number(amountIn)/10**Number(decimalsIn)} ${toTokenName(tokenIn)} to ${toTokenName(tokenOut)}..`);
    ...
    console.log(`Input token balance: ${ethers.formatUnits(tokenInBalance, decimalsIn)} ${toTokenName(tokenIn)}`);
    if (BigInt(tokenInBalance) < BigInt(amountIn)) {
        throw new Error(`Insufficient balance: ${ethers.formatUnits(tokenInBalance, decimalsIn)}`);
    }

    const sBalance = await provider.getBalance(walletAddress);
    const minSBalance = ethers.parseUnits('0.01', 18);
    if (sBalance < minSBalance) {
        throw new Error(`Insufficient S balance: ${ethers.formatUnits(sBalance, 18)} S`);
    }
    ...
    console.log(`Swapping ${ethers.formatUnits(amountIn, decimalsIn)} ${toTokenName(tokenIn)}...`);
...
module.exports = swapTokensShadow;